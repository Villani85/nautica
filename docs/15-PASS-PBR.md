...
| 28 ago, 10:10 | — | **il confronto sito-Cycles misurava due mondi anche dopo la camera: mancavano l'ambiente e la curva tonale.** `cuoci.py` costruiva per la nave un gradiente SUO con l'idea giusta (carta sopra, acqua sotto) ma senza il disco del sole -- una seconda implementazione dell'ambiente. E rendeva con `view_transform = 'AgX'` a **-1 EV** mentre il sito disegna in **ACESFilmic** a esposizione 1: due curve diverse e uno stop |
| 28 ago, 10:10 | — | curati: `node strumenti/esporta-ambiente.mjs` scrive la tela VERA (quella che il sito da' al PMREMGenerator) e `AMBIENTE_SITO=1` la passa a Blender; three ha AgX anche lui, quindi il confronto allinea la curva invece di correggere a posteriori. **La rotazione e' 0 ed e' DERIVATA**: con la conversione d'assi sito->Blender (x,y,z)->(x,-z,y), la u di Blender diventa esattamente quella di three e per un versore atan2(y,hypot(x,z)) = asin(y). I -55 gradi dell'HDRI d'officina qui avrebbero messo il sole da un'altra parte |
| 28 ago, 10:10 | — | **e il risultato dice finalmente dove il tempo reale perde.** Sovrastruttura: scarto medio da 30,2 a **20,4** livelli, di cui esposizione -14,5 e struttura **14,2**. Sopra la linea d'acqua l'esposizione coincide a **+0,8 livelli**, che e' la prova che l'allineamento era la correzione giusta, e resta **39,9 di struttura**. Quindi non e' la sovrastruttura a perdere: e' lo SCAFO |
| 28 ago, 10:10 | — | il PNG dell'ambiente NON e' committato apposta: e' derivato, e se la tavolozza cambiasse resterebbe indietro in silenzio falsando proprio il confronto. `cuoci.py` si ferma con un errore e dice il comando |
| 28 ago, 11:00 | — | **quarto strato: gli impianti di luce erano due.** Camera, ambiente e curva tonale ormai allineati, ma il sito ha emisferica 2,7 + sole 3,6 + controluce 1,4 + puntiforme 3,2, e `cuoci.py` due luci ad area (120 e 45 volte d²). Finche' restano accesi, nessuno scarto e' attribuibile alla resa. Aggiunto `SENZA_LUCI=1` al confronto, che sull'altro lato corrisponde a `LUCE=0` |
| 28 ago, 11:00 | — | **e con tutti e quattro allineati il risultato e' netto**: sovrastruttura scarto **9,7** livelli (esposizione -1,2, struttura 12,3) -- cioe' il tempo reale COMBACIA col path tracer entro dieci livelli. Scafo emerso: scarto **32,6**, esposizione **+20,5**, struttura 35,4 |
| 28 ago, 11:00 | — | **il difetto e' isolato e ha una firma precisa.** Campionando quattro zone dello scafo contro due della tuga: tuga -3/-1/+2 e +7/+6/+7, scafo **+37/+37/+38**, **+40/+43/+46**, +33/+31/+31, +44/+40/+37. Un sollevamento **uniforme sui tre canali**, senza spostamento di tinta, **su un solo materiale**. Non e' la buccia d'arancia: spegnendola il residuo va da 41,2 a 40,8, anche se da sola cambia 3.249 pixel di 50 livelli -- rumore ad alta frequenza sopra una differenza sistematica |
| 28 ago, 11:00 | — | e non e' nemmeno il materiale, letto dal vivo nel browser: `scafo` e' `#707c82`, met 0, rug 0,13, envMap a intensita' 1, emissivo nero, nessuna lightmap. Il JSON che Blender legge porta gli stessi identici numeri. **Aperto**: da dove vengono quei 38 livelli. I sospetti non ancora esclusi sono il `DoubleSide` dello scafo contro l'`interno` che Blender salta apposta, e la differenza fra l'irradianza prefiltrata di three (PMREM su un equirettangolare da 512x256) e il campionamento diretto di Cycles a rugosita' 0,13 |
| 28 ago, 11:40 | — | **CORREZIONE al giro precedente: il +38 livelli sullo scafo era misurato su riquadri scelti a occhio.** Sulla maschera esatta del materiale -- ottenuta dipingendolo di emissivo -- lo scarto e' **+26,6**, cioe' luminanza 53,1 contro 26,5: il sito e' **2,51 volte** piu' luminoso. La conclusione regge, il numero no. Terzo errore identico della notte, dopo il vetro e la banda della coperta |
| 28 ago, 11:40 | — | e una seconda trappola dentro la stessa sonda: leggevo i pixel disegnando la tela WebGL su un canvas 2D. Senza `preserveDrawingBuffer` quella strada restituisce un fotogramma vecchio, e **sei varianti diverse davano sei valori IDENTICI** -- compreso dipingere lo scafo di rosso. Adesso si legge con `gl.readPixels`, e c'e' una variante `PROVA_ROSSO` che DEVE spostare il numero |
| 28 ago, 11:40 | — | **LA CAUSA: `side: DoubleSide` su `scafo` (`materiali.js:63`).** Lo scafo e' un GUSCIO aperto -- un loft, non un solido -- quindi three ne disegna anche la parete interna lontana, che si somma alla vicina. Misurato per varianti sulla maschera: com'e' ora **2,51x** Cycles, con `FrontSide` **1,02x**, senza ambiente 0,09x (quindi e' l'ambiente a illuminarlo), senza buccia d'arancia 2,62x (quindi non e' lei) |
| 28 ago, 11:40 | — | **ma la cura non e' `FrontSide`**: portando il piano di sezione DENTRO lo scafo, FrontSide apre **9.786 pixel di buco**. Il DoubleSide regge il taglio. Le strade vere sono dare allo scafo uno spessore di parete -- che e' quello che D32 prescrive gia' per i tappi di sezione -- oppure scartare le facce posteriori tranne dove il piano taglia. **Non spedito**: e' un difetto misurato e spiegato, con una tensione di progetto vera da sciogliere |
| 28 ago, 11:40 | — | nuovo strumento `strumenti/varianti.mjs`: misura una variante per volta sulla maschera del materiale stesso, con la prova del rosso incorporata |
| 28 ago, 12:00 | — | **il primo feedback esterno utile, e aveva ragione lui.** Attaccava il mio «la pinna sta a fondo corsa nel 94% dei fotogrammi»: si e' scritto uno script, ha contato a piu' velocita' e ha trovato un massimo del 87%, **0% a 12 nodi** -- l'andatura di servizio (`V_RIF`), che e' quella con cui il sito si apre |
| 28 ago, 12:00 | — | **verificato nel sito, non nel simulatore, e confermato**: mare 4 a 12 nodi fondo corsa **0,0%** con picco pinna 9,1 gradi; mare 5 a regime **0,0%** con picco **16,0** su un fine corsa di 25. Il mio 94% lo stampava `collaudo-manopola`, che misura SUBITO DOPO aver spento e riacceso l'interruttore -- cioe' dentro la rampa che gonfia l'ampiezza alla carena nuda. **Era un transitorio dentro una prova, riportato come il comportamento del sito** |
| 28 ago, 12:00 | — | corretto in tre posti: il cancello adesso stampa «fondo corsa il N% *del transitorio*», sopra `K = 17` c'e' il conto che spiega perche' non satura al punto di lavoro (rollio residuo 1,4 gradi, omega di picco 1,24 gradi/s contro una soglia A_MAX/K di 1,47), e la risposta sta in `feedback/CHIEDO.md` |
| 28 ago, 12:00 | — | e una cosa che non sapevo, dal revisore: **K costante non e' una mancanza di gain scheduling**, perche' l'autorita' della pinna scala gia' col quadrato della velocita', quindi il prodotto scala con v² comunque |
| 28 ago, 13:00 | — | **CORREZIONE: il DoubleSide non era la causa del 2x sullo scafo.** La misura che me lo faceva credere usava una maschera del materiale che comprendeva anche i pixel delle facce POSTERIORI: passando a `FrontSide` quei pixel li prende `interno`, che e' scuro, e la media scendeva. Scendeva perche' cambiava CHI li dipinge. Sui pixel della superficie ESTERNA, prima e dopo la build: **53,07 e 53,09** |
| 28 ago, 13:00 | — | il `FrontSide` si tiene lo stesso, ma per un'altra ragione e con un altro numero: disegnare l'interno di un guscio come se fosse illuminato dal cielo e' sbagliato, e `interno` esiste apposta. Misurato: cambia 12.077 pixel nella zona bassa, di cui solo **263** hanno materia anche in Cycles -- Cycles 59,0, prima 70,8 (1,20x), dopo 56,7 (**0,96x**). Miglioramento piccolo su base sottile |
| 28 ago, 13:00 | — | e un sesto errore della stessa famiglia, scoperto strada facendo: contavo come «buco» ogni pixel con la somma dei canali sotto 12, e `interno` e' `#1b2224`, somma 97 -- scurissimo e presente. Cosi' i «9.786 pixel di buco» che avevo attribuito al FrontSide erano **zero**, misurati sull'alfa. **Il vuoto si riconosce dall'alfa, non dalla luminosita'** |
| 28 ago, 13:00 | — | **il 2x resta aperto.** Escluso: buccia d'arancia (2,62x spegnendola), `interno` (2,51x nascondendolo), parametri del materiale (identici al JSON che Blender legge), `side`. Sospetto rimasto: l'irradianza prefiltrata di three -- PMREM su un equirettangolare di soli 512x256 -- contro il campionamento diretto di Cycles a rugosita' 0,13 |
| 28 ago, 13:40 | — | **il numero vero: lo scafo e' 1,29 volte piu' chiaro del path tracer, non 2,5.** Sulle sole facce ANTERIORI -- Cycles 39,0 contro sito 50,4, su 10.405 pixel. Il 2,5x era interamente un artefatto della maschera, che comprendeva anche le facce posteriori: li' il sito disegna una parete interna illuminata e Cycles non disegna niente, perche' per il ritratto della nave salta `interno` apposta |
| 28 ago, 13:40 | — | **la regola, che mi e' costata tre conclusioni sbagliate di fila**: una maschera ottenuta dipingendo un materiale di emissivo da i pixel che sono suoi, comprese le facce POSTERIORI. Su un guscio aperto quelle non sono la superficie che si sta confrontando. Verificare la maschera non basta: bisogna verificare che contenga la SUPERFICIE giusta |
| 28 ago, 13:40 | — | e il 29% resta aperto, con quattro sospetti esclusi per misura (buccia, `interno`, parametri del materiale, `side`). Quello rimasto: l'irradianza prefiltrata di three -- PMREM su un equirettangolare di 512x256 -- contro il campionamento diretto di Cycles. A rugosita' 1 il rapporto resta alto, quindi sembra la parte DIFFUSA e non lo speculare |
| 28 ago, 09:45 | — | **il vetro: il colore e' la leva piu' forte, non l'ambiente.** Misurato sulla maschera esatta di `sovra_vetro` (324 px alla camera del ritratto): com'e' 56,5; ambiente x2 -> 68,3 (+11,8); ambiente x4 -> 84,5; colore `#1a3330` -> 77,7 (**+21,2**); `#3a5050` -> 111,1. Il feedback esterno aveva ragione che sono due contributi ma li aveva ordinati al contrario. **Non cambiato**: `0x061412` e' una decisione di tavolozza, non un difetto. E nota di scala: 324 pixel, cioe' molto meno di quanto pesasse nella domanda |
| 28 ago, 09:55 | — | **il 29% dello scafo: causa nominata, ed e' l'irradianza DIFFUSA di three.** Escluso prima il sospetto della risoluzione: portando l'ambiente da 512x256 a 2048x1024 e ri-renderizzando anche Blender, il rapporto va da 1,292 a 1,315 -- non si muove, e Cycles resta 39,0 perche' integra lo stesso contenuto. Ripristinato 512 |
| 28 ago, 09:55 | — | e non e' nemmeno lo speculare a incidenza radente, che era l'ipotesi suggerita dal fatto che il rapporto **cresce verso prua** (1,14 a poppa, 1,43 a prua) e **in cima allo scafo** (2,53 nella banda y 310-335). Misurato sulla maschera delle anteriori: **solo speculare 13,1**, cioe' il 26% del totale, e irruvidire PEGGIORA -- rugosita' 0,5 da 1,49x e rugosita' 1 da 1,41x contro l'1,28x di serie |
| 28 ago, 09:55 | — | quindi l'eccesso sta nella parte **diffusa**: `getIBLIrradiance` legge l'ultimo livello del PMREM, che e' un'approssimazione dell'integrale coseno-pesato, e su un cielo a forte contrasto -- carta chiara sopra, acqua scura sotto, che e' esattamente il nostro -- sovrastima. Cycles lo integra esatto. **Non curato**: la cura vera e' separare l'irradianza (un `LightProbe` con armoniche sferiche) dallo speculare, e in three un materiale con `envMap` prende comunque il diffuso da li'. Compensare con `envMapIntensity` sarebbe un trucco che toccherebbe anche lo speculare |
| 28 ago, 10:30 | — | **tutti i rapporti che avevo pubblicato erano in spazio DISPLAY, dopo AgX, e non sono confrontabili fra loro.** Me ne sono accorto sostituendo il cielo con uno piatto: peggiorava anche la **sovrastruttura**, da 0,988x a 1,138x, cioe' una zona che prima combaciava. Un cambiamento che tocca anche cio' che era giusto non e' un difetto del soggetto, e' il metro. AgX e' non lineare: lo stesso rapporto di radianza da rapporti diversi a luminosita' diverse |
| 28 ago, 10:30 | — | aggiunto `LINEARE=1` ai due lati -- `NoToneMapping` nel sito, view transform `Standard` in Blender -- cosi' i valori sono sRGB invertibili esattamente. E **`ESPOSIZIONE=0`**, perche' `cuoci.py` rende a **-1 EV** di serie e in `Standard` quella meta' resta: senza, i rapporti uscivano esattamente il doppio (scafo 2,74x invece di 1,37x) |
| 28 ago, 10:30 | — | **i numeri veri, in lineare a esposizione 0**: scafo Cycles 0,0564 contro sito 0,0772 = **1,370x**; sovrastruttura Cycles 0,3267 contro sito 0,2605 = **0,797x**. Cioe' **due errori di segno OPPOSTO**, che la curva nascondeva mostrando la sovrastruttura a 1,0 |
| 28 ago, 10:30 | — | **e i segni opposti confermano la diagnosi e la precisano.** La sovrastruttura guarda il cielo chiaro ed esce piu' SCURA; lo scafo guarda il mare scuro ed esce piu' CHIARO. Non un eccesso uniforme: una **regressione verso la media**, che e' esattamente cio' che fa un'irradianza prefiltrata su un ambiente a forte contrasto. Spiega anche perche' la rugosita' non la tocca (e' il termine diffuso), perche' la risoluzione non la tocca (e' l'approssimazione, non il campionamento) e perche' il cielo piatto la riduceva |
| 28 ago, 11:20 | — | **l'irradianza in armoniche sferiche: costruita, verificata viva, e NON spedita — perche' non cambia niente.** Era la cura che la diagnosi chiedeva: three calcola l'irradianza diffusa come `E = pi * L` sul livello piu' sfocato della mappa (`getIBLIrradiance`, letto nel sorgente), che e' esatto solo per un cielo uniforme. Nove coefficienti la darebbero vera |
| 28 ago, 11:20 | — | fatta per intero: `armonicheAmbiente()` integra la tela equirettangolare con la base e la normalizzazione di three, e `irradianzaVera(m, arm)` sostituisce il pezzo `lights_fragment_maps` dentro il materiale invece di aggiungere un `LightProbe` -- che essendo una luce illuminerebbe anche i materiali senza ambiente. **Verificata contro l'integrale a forza bruta**: scarto 0,1-0,2% su quattro normali su cinque (la quinta, verso il basso, +11% su un valore quasi nullo) |
| 28 ago, 11:20 | — | **e verificata VIVA prima di credere al risultato**: azzerando le nove uniformi lo scafo crolla da 73,1 a 33,2, quindi lo shader le usa davvero. Con esse: scafo **1,368x** contro 1,370x, sovrastruttura 0,801x contro 0,797x. **Le due formulazioni coincidono entro lo 0,15%**: su QUESTO cielo l'approssimazione di three e' gia' giusta, e la mia diagnosi era sbagliata |
| 28 ago, 11:20 | — | tre trappole pagate costruendola, tutte da ricordare: **`onBeforeCompile` riceve gli `#include` NON espansi** (per questo tutte le patch del repo sostituiscono nomi di include e non righe); **viene chiamato anche per lo shader di profondita' delle ombre**, dove `lights_fragment_maps` non c'e' e non deve esserci; e **`this` dentro `onBeforeCompile` e' il materiale VERO**, che sui cloni non e' quello catturato nella chiusura -- col marcatore sull'originale il collaudo diceva «scafo NO» mentre la patch era applicata |
| 28 ago, 11:20 | — | **resta aperto il 37% sullo scafo e il -20% sulla sovrastruttura.** Sospetto nuovo, non ancora provato: three non ha **occlusione dell'ambiente** -- Cycles ombreggia il cielo con lo sbalzo della coperta e con la curvatura dello scafo, three no. Spiegherebbe lo scafo piu' chiaro; NON spiega la sovrastruttura piu' scura, che quindi ha un'altra causa |
| 28 ago, 11:45 | 2 | **il feedback esterno ha smontato il conto dei byte, e aveva ragione.** Dicevo «+40 KB netti» per lo scambio alta->bassa, misurando l'uscita grezza di gltfpack. Sul FILO e' il contrario: prima 129,9 KB brotli, dopo **343,4**, cioe' **+213,5**. Due cause: la geometria meshopt si comprime ancora 2,4 volte con brotli (309,4 -> 129,9), una texture webp per niente (218,1 -> 216,0, il 99%) |
| 28 ago, 11:45 | 2 | **e la via d'uscita e' misurata, non scelta.** Il recupero della normale e' lo STESSO alle tre risoluzioni -- 61,7% a 2048, 63,6% a 1024, 62,5% a 512, differenze dentro il rumore del render -- mentre i byte sul filo sono 343,4 / 210,0 / **160,7**. Si cuoce a 2048 dove i cancelli sono tarati e si SPEDISCE a 512, come si faceva gia' per l'occlusione. Netto contro l'alta: **+30,8 KB**, non +213 |
| 28 ago, 11:45 | — | **e la seconda voce era una conseguenza della prima, piu' seria.** Tre commenti dicevano ancora «223 KB», e `guasto.js` ci TARA sopra la soglia della spia d'attesa: 223 KB a 400 kbit/s sono 4,46 s, con `ATTESA = 6000` di margine. Coi 343,4 diventavano **6,87 s**, cioe' la spia si sarebbe accesa proprio sulla connessione per cui e' scritta per NON accendersi. Adesso sono 161 KB = **3,21 s**, margine piu' largo di quando il commento fu scritto |
| 28 ago, 11:45 | — | cifre della pagina aggiornate: modelli 558 -> **374 KB**, totale prima del primo fotogramma 2,17 -> **1,99 MB**, percorso critico 17,0 -> **17,3 KB** (cresciuto per la prosa nuova, che spiega lo scambio). E la pipeline ora e' un comando che gira davvero da capo a fondo: `pwd -W`, perche' Blender legge un `-P` in forma MSYS come percorso relativo |
| 28 ago, 11:50 | — | **Voce 3 del feedback, sullo spettro del mare: verificata, e l'esito e' a meta'.** Replicato l'integratore e VALIDATO -- tre righe, replica 90,83% contro `riduzioneVera` 90,78%, scarto 0,05 punti. Il caso limite conferma il suo meccanismo: **una riga sola esattamente a risonanza da' 98,0%**, contro il 98,1% che riportava lui |
| 28 ago, 11:50 | — | **ma la sua conclusione no.** I miei spettri JONSWAP a varianza uguale danno 75-88,5% (picco 1,0 W: 87,0%), i suoi 83-96,5% (picco 1,0 W: 96,5%): otto punti di differenza con lo stesso integratore validato da tutti e due. E la banda stretta mostra perche': **non e' monotona** -- +-5% da' 97,7%, +-15% **78,7%**, +-40% 81,3%, +-80% 87,0%. Con molte righe vicine la forzante BATTE, e il periodo del battimento e' confrontabile con la finestra di misura (45+90 s): il numero dipende dalla finestra, non solo dalla fisica |
| 28 ago, 11:50 | — | quindi **non e' stabilito che il sito sia conservativo, ne' il contrario** -- ma la sua osservazione di fondo regge, ed e' quella che conta: il 90,8% appartiene a QUELLA forzante, non alla nave. La pagina adesso lo dice, con la fascia misurata: 98% con tutta l'energia sul periodo di rollio, 83% con un mare largo su periodi corti, 91% con le tre armoniche in uso |
| 28 ago, 12:10 | — | **CORREZIONE: la sovrastruttura non e' scura del 20%, combacia.** Quel numero veniva dalla banda y 150-300, che comprende teak, vetro, montanti, parapetti e sfondo. Sulla maschera ESATTA del materiale, `sovra_guscio` da' **1,007x** -- entro lo 0,7%. **Il difetto e' uno solo: lo scafo.** E cade con esso tutto il ragionamento sui «due segni opposti» |
| 28 ago, 12:10 | — | e cadono altri due sospetti. **L'occlusione dello scafo c'e' ed e' corretta** (512x512, canale 0, UV presenti): togliere `aoMap` non cambia niente perche' sulle murate esposte al cielo l'occlusione e' ~1, che e' giusto. E i due materiali `scafo` senza mappa sono i **tappi di prua e poppa**, che usano il materiale condiviso apposta -- il commento in `nave.js` lo dichiara: con la mappa leggerebbero l'atlante del guscio |
| 28 ago, 12:10 | — | **stato: un materiale, +36%, otto sospetti esclusi ognuno con una misura** -- buccia d'arancia, guscio `interno`, parametri del materiale, `side`, risoluzione dell'ambiente, speculare a incidenza radente, irradianza in armoniche sferiche, mappa d'occlusione. Un'osservazione che resta senza spiegazione: la separazione fra i due materiali dentro ciascun motore e' **5,33x in Cycles e 3,95x nel sito**, contro un rapporto dei soli albedo di 4,27 -- il sito comprime, Cycles allarga |
